...
import com.dami.fileexplorer.view.FileListItem.ModeCallback;
import com.dami.fileexplorer.view.FileViewActivity.SelectFilesCallback;
import com.dami.fileexplorer.view.PathGallery.IPathItemClickListener;
...
public class FileViewInteractionHub implements IOperationProgressListener, IPathItemClickListener {
    ...
    private final String ENC_FILENAME = "enc_source.txt";
    private final String DEC_FILENAME = "dec_source.txt";
    ...
    public FileViewInteractionHub(IFileInteractionListener fileViewListener, int i) {
        assert (fileViewListener != null);
        mFileViewListener = fileViewListener;
		mTabIndex = i;
        setup();
    ...
    private void setupPathGallery()
    {
      ...
      mPathGallery = ((PathGallery)mFileViewListener.getViewById(R.id.path_gallery_nav));
    ...
    private void setupNaivgationBar() {
        ...
        mNavigationBarText.setVisibility(View.GONE);
    ...
    /*
    private void onOperationEncrypt() {
    	//mEncDecBusiness.encFile("1234567890", CommonUtils.SD_PATH + FILENAME, CommonUtils.SD_PATH + ENC_FILENAME);
    	File dirFirstFolder = new File(mContext.getResources().getString(R.string.encrypt_path));
    	...
    	String path = getSelectedFileList().get(0).filePath.toString();
    	...
    	mEncDecBusiness.encFile(CommonUtils.groupId, path, CommonUtils.SD_PATH + mContext.getResources().getString(R.string.encrypt) + filename);
    	mFileOperationHelper.Delete(getSelectedFileList());
		clearSelection();
    }    
    
    private void onOperationDecrypt(){
    	File dirFirstFolder = new File(mContext.getResources().getString(R.string.decrypt_path));
    	if(!dirFirstFolder.exists())  
        { //如果该文件夹不存在，则创建文件夹    
          dirFirstFolder.mkdirs();
        } 
    	...
    	mEncDecBusiness.decFile(CommonUtils.SD_PATH + mContext.getResources().getString(R.string.decrypt) + realfilename, path);
    	mFileOperationHelper.Delete(getSelectedFileList());
    	clearSelection();
    }

        public void onOperationEncrypt() {
        final ArrayList<FileInfo> selectedFiles = new ArrayList<FileInfo>(getSelectedFileList());
        File dirFirstFolder = new File(mContext.getResources().getString(R.string.encrypt_path));
        if(!dirFirstFolder.exists())
        {
            dirFirstFolder.mkdirs();
        }
        if (mFileOperationHelper.Encrypt(selectedFiles,dirFirstFolder)) {
            Log.e(LOG_TAG, "onOperationEncrypt: ");
            showProgress(mContext.getString(R.string.operation_encrypt));
        }
        clearSelection();
    }
        ...
        public void onOperationEncrypt() {
        final ArrayList<FileInfo> selectedFiles = new ArrayList<FileInfo>(getSelectedFileList());
        final File dirFirstFolder = new File(mContext.getResources().getString(R.string.encrypt_path));
        if(!dirFirstFolder.exists())
        {
            dirFirstFolder.mkdirs();
        }
    ...
    */
    ...
    public void onOperationDecrypt() {
        ...
        if (mFileOperationHelper.Decrypt(selectedFiles,dirFirstFolder)) {
            Log.e(LOG_TAG, "onOperationDecrypt: ");
            showProgress(mContext.getString(R.string.operation_decrypt));
        }
        //clearSelection();
    }
    ...
    */
    ...
    private void updateNavigationPane() {
        ...
        Log.e("william","william mCurrentPath is:"+mCurrentPath);
        Log.e("william","william path is:"+mFileViewListener.getDisplayPath(mCurrentPath));
    }
    ...
    private OnCreateContextMenuListener mListViewContextMenuListener = new OnCreateContextMenuListener() {
        ...
        public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
            ...
            addMenuItem(menu, GlobalConsts.MENU_ENCRYPT, 0, R.string.operation_encrypt);
            addMenuItem(menu, GlobalConsts.MENU_DECRYPT, 0, R.string.operation_decrypt);
        ...
        public boolean onMenuItemClick(MenuItem item) {
            ...
            switch (itemId) {
                ...
                case GlobalConsts.MENU_FAVORITE:
                    onOperationFavorite();
                    break;
                case GlobalConsts.MENU_ENCRYPT:
                    onOperationEncrypt();
                    break;
                case GlobalConsts.MENU_DECRYPT:
                    onOperationDecrypt();
                    break; 
                case MENU_SETTING:
                    onOperationSetting();
    ...
    private com.dami.fileexplorer.view.FileViewInteractionHub.Mode mCurrentMode;
    ...
    */
	...
	public void onPathItemClickListener(String paramString) {
		...
		mCurrentPath = mFileViewListener.getRealPath(paramString);
		if (mTabIndex == 0) {
			//mFileViewListener.onOperation(4);
			onBackPressed();
		}else if(mTabIndex == 1){
			refreshFileList();
		}
