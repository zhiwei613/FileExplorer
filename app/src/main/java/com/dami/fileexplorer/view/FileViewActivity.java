...
import com.dami.fileexplorer.ActivitiesManager;
import com.dami.fileexplorer.R;
import com.dami.fileexplorer.adapter.FileListAdapter;
...
import com.dami.fileexplorer.view.FileViewInteractionHub;
...
public class FileViewActivity extends Fragment implements
    ...
    private OnClickListener mStorageVolumeClick = new OnClickListener() {
        public void onClick(View view) {
            showVolumesList(false);
            Pair pair = (Pair) view.getTag();
            if (!((String) pair.first).equals(mVolumePath)) {
                mVolumePath = (String) pair.first;
                mVolumeDescription = (String) pair.second;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mVolumeSwitch.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                int visibile = mRootView.findViewById(R.id.dropdown_navigation).getVisibility();
                if (visibile == 8) {
					showVolumesList(true);
                    buildVolumesList();
					visibile = mRootView.findViewById(R.id.dropdown_navigation).getVisibility();
                } else if (visibile == 0) {
                    showVolumesList(false);
                }
            }
        ...
        if (!TextUtils.isEmpty(rootDir)) {
            if (baseSd && FileViewActivity.sdDir.startsWith(rootDir)) {
                rootDir = FileViewActivity.sdDir;
            }
        } else {
            rootDir = baseSd ? FileViewActivity.sdDir : GlobalConsts.ROOT_PATH;
        }

		ArrayList sortedMountVolumeList = StorageHelper.getInstance(mActivity).getSortedMountVolumeList();
		Iterator it = sortedMountVolumeList.iterator();
		if (sortedMountVolumeList != null && sortedMountVolumeList.size() != 0) {
			while (it.hasNext()) {
				storageVolume = (StorageVolume) it.next();
				mVolumePath = storageVolume.getPath();
				mVolumeDescription = storageVolume.getDescription(mActivity);
			}
		}
	
        mFileViewInteractionHub.setRootPath(mVolumePath);

        currentDir = FileExplorerPreferenceActivity.getPrimaryFolder(mActivity);
        Uri uri = intent.getData();
        if (uri != null) {
            if (baseSd && FileViewActivity.sdDir.startsWith(uri.getPath())) {
                currentDir = FileViewActivity.sdDir;
            } else {
    ...
    public boolean onBack() {
		...
		if(mPreviousPath.equals("/storage/emulated/0")){
			return false;
		}
    ...
    private void notifyFileChanged(Boolean bool) {
        //showVolumesList(false);
        //updateVolumeSwitchState();
    }	
    ...
    private void updateVolumeSwitchState() {
        if (mVolumeSwitch != null) {
            StorageHelper.getInstance(mActivity).getMountedVolumeCount();
            ArrayList sortedMountVolumeList = StorageHelper.getInstance(mActivity).getSortedMountVolumeList();
            ArrayList arrayList = new ArrayList();
            for (int i = 0; i < sortedMountVolumeList.size(); i++) {
                StorageVolume storageVolume = (StorageVolume) sortedMountVolumeList.get(i);
    ...
    private void buildVolumesList() {
        LinearLayout linearLayout = (LinearLayout) mRootView.findViewById(R.id.dropdown_navigation_list);
        ...
        if (sortedMountVolumeList != null && sortedMountVolumeList.size() != 0) {
            ...
            while (it.hasNext()) {
                storageVolume = (StorageVolume) it.next();
                if (storageVolume.getStorageId() != 0) {
                    linearLayout.addView(createStorageVolumeItem(storageVolume.getPath(), storageVolume.getDescription(mActivity)));
                }
            }
        }
    }	
    ...
    private View createStorageVolumeItem(final String str, String str2) {
        View inflate = LayoutInflater.from(mActivity).inflate(R.layout.dropdown_item, null);
        ...
        if (storageVolume.getPath().equals(internalPath)) {
            imageView.setImageDrawable(getResources().getDrawable(R.drawable.storage_internal_n));
        } else if (storageVolume.getDescription(mActivity).toString().contains("SD")) {
            imageView.setImageDrawable(getResources().getDrawable(R.drawable.storage_sd_card_n));
        } else if (storageVolume.getDescription(mActivity).toString().contains("usbotg")) {
            imageView.setImageDrawable(getResources().getDrawable(R.drawable.storage_usb_n));
        }
        imageView = (ImageView) inflate.findViewById(R.id.unmount_btn);
        if (str.equals(Util.SD_DIR)) {
			int checkpermission = mActivity.checkPermission("android.permission.MOUNT_UNMOUNT_FILESYSTEMS", Process.myPid(), Process.myUid());
        ...
        inflate.setOnClickListener(mStorageVolumeClick);
        inflate.setTag(new Pair(str, str2));
        return inflate;
    }

    private void showVolumesList(boolean z) {
        mRootView.findViewById(R.id.dropdown_navigation).setVisibility(z ? View.VISIBLE : View.GONE);
    }
    ...
    public void onMountStateChange(int i) {
        if (i == 1) {
            notifyFileChanged(Boolean.valueOf(true));
        } else {
            notifyFileChanged(Boolean.valueOf(false));
        }
    }	
    ...
    public String getDisplayPath(String path) {
    	/*
        if (path.startsWith(FileViewActivity.sdDir) && !FileExplorerPreferenceActivity.showRealPath(mActivity)) {
            return getString(R.string.storage_phone) + path.substring(FileViewActivity.sdDir.length());
        } else {
            return path;
        }
        */
        return path.startsWith(mVolumePath) ? mVolumeDescription + path.substring(mVolumePath.length()) : path;
    }
    ...
    public String getRealPath(String displayPath) {
        ...
        if (displayPath.startsWith(perfixName)) {
            return FileViewActivity.sdDir + displayPath.substring(perfixName.length());
        } else {
            return displayPath;
        }
        */
        return displayPath.startsWith(mVolumeDescription) ? mVolumePath + displayPath.substring(mVolumeDescription.length()) : displayPath;
    }
